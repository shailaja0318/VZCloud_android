...
public class LocalyticsAppLaunchBackground extends BaseTestClass {
	...
	public void testLocalyticsAppLaunchBackground() throws Exception {
		...
		TestCase.assertTrue("Localytics of " + vz_strings.logs_appLaunch + " is not 1 in logs", localyticsHelper
				.getPatternMatch(logs, vz_strings.logs_tagEvent + ": " + vz_strings.logs_appLaunch) == 1);

		TestCase.assertTrue("Localytics of " + vz_strings.logs_networkType + " does not exist",
				localyticsHelper.getPatternMatch(logs,
						"\"" + vz_strings.logs_networkType + "\" = \"" + vz_strings.logs_networkTypeWifi + "\"") == 1);

		TestCase.assertTrue("Localytics of " + vz_strings.logs_source + " does not exist", localyticsHelper
				.getPatternMatch(logs, vz_strings.logs_source + " = " + vz_strings.logs_sourceDirect) == 1);
